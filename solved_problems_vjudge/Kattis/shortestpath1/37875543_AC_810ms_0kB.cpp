...
void solve() 
{
    int n, m, q, s; // n of nodes in the graph, n of edges, queries, starting node
    while(cin >> n >> m >> q >> s && !(n == 0 && m == 0 && q == 0 && s == 0))
    {
        ...
        int q_node; // nodo inicial
        for (int i = 0; i < q; i++) // por cada query
        {
            distances.assign(n, INF);
            cin >> q_node;
            dijkstra(adj_list, distances, s, q_node);
            (distances[q_node] == INF) ? cout << "Impossible" : cout << distances[q_node];
            cout << endl;
